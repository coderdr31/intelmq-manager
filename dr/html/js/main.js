    ...
    for(bot_group in config) {
        ...
        switch (bot_group)
        {
            ...
            case "Output":
                ...
                break;
            default:
                bot_group_cn=bot_group;
        }

        group_title.innerHTML = bot_group_cn + '<span class="fa arrow"></span>';
...
function create_form(title, data, callback){
    ...
    table.innerHTML="<p>请从左边选取一个Bot</p>";
...
//                shape: 'database',
        ...
        onAdd: function(data,callback) {
            create_form("增加节点", data, callback);
        },
        onEdit: function(data,callback) {
            create_form("编辑节点", data, callback);
        ...
        onConnect: function(data,callback) {
            ...
            if (!available_neighbor) {
                if (neighbors.length == 0) {
                    show_error("节点类型 " + nodes[data.from].group + " 不能连接其它节点");
                } else {
                    show_error('节点类型 ' + nodes[data.from].group + ' 只能连接以下类型的节点: ' + neighbors.join());
                }
                return;
            }
